...
async def view_repo_files(ctx: RunContext[GitHubDependencies]) -> str:
    ...
    """
    github = GitHubConn()
    files = github.get_repo_files(ctx.deps.repo_name, ctx.deps.path)
    result = [f"Files in {ctx.deps.repo_name}/{ctx.deps.path or 'root'}:"]
    for file in files:
...
async def view_pull_request(ctx: RunContext[GitHubDependencies]) -> str:
    ...
    """
    github = GitHubConn()
    pr = github.get_pull_request(ctx.deps.repo_name, ctx.deps.pr_number)
    return (
...
async def view_pr_comments(ctx: RunContext[GitHubDependencies]) -> str:
    ...
    """
    github = GitHubConn()
    comments = github.get_pr_comments(ctx.deps.repo_name, ctx.deps.pr_number)
    if not comments:
        return f"No comments found on PR #{ctx.deps.pr_number}"

    result = [f"Comments on PR #{ctx.deps.pr_number}:"]
...
async def view_branches(ctx: RunContext[GitHubDependencies]) -> str:
    ...
    """
    github = GitHubConn()
    branches = github.get_branches(ctx.deps.repo_name)
    logfire.info(f"Branches in {ctx.deps.repo_name}: {branches}")
    ...
    logfire.info(f"Branches in {ctx.deps.repo_name}: {result}")
    return "\n".join(result)


async def list_all_pull_requests(ctx: RunContext[GitHubDependencies], state: str = "all") -> str:
    ...
    """
    github = GitHubConn()
    prs = github.list_pull_requests(ctx.deps.repo_name, state)
    if not prs:
        return f"No pull requests found in {ctx.deps.repo_name}"

    result = [f"Pull Requests in {ctx.deps.repo_name}:"]
    result.extend(
        f'\n#{pr["number"]}: {pr["title"]}'
    ...
    logfire.info(f"Pull Requests in {ctx.deps.repo_name}: {result}")
    return "\n".join(result)
