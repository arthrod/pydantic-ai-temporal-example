...
async def view_repo_files(ctx: RunContext[GitHubDependencies], path: str = '') -> str:
    ...
    """
    github = GitHubConn()
    files = github.get_repo_files(ctx.deps.repo_name, path)
...
github_agent = Agent(
    ...
    system_prompt=(
        ...
        'You should FOLLOW THE INSTRUCTIONS CAREFULLY AND MERELY USE THE TOOLS.'
    ),
)
    ...
    async def main():
        try:
            ...
            result = await github_agent.run(
                'Show me the branches in this repository entitled pydantic-ai-temporal-example', deps=deps_instance
            )
